...
class Reservation {
    ...
    get info (){
        return `${this.customer.info} - ${this.date.toLocaleString()} - ${this.guests}`;
    }

    static validateReservation(fecha, personas){
        const fechaActual = new Date();
        const fechaReserva = new Date(fecha);
        if (personas > 0 && fechaReserva > fechaActual) return true;
        return false;
